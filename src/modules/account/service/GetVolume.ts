...
export class GetVolume {
  ...
  public execute = async ({
    userId,
  }: IRequest): Promise<IResponseGetVolume> => {
    ...
    const cryptoSales =
      await this.cryptoTransactionRepository.listAllCryptoSales({
        accountId: account.id,
      });

    return {
      bought: investments.reduce(
        (sum: number, element: CryptoTransactionModel): number => {
          return sum + element.quantity;
        },
        0,
      ),
      sold: cryptoSales.reduce(
        (sum: number, element: CryptoTransactionModel): number => {
          return sum + element.quantity;
        },
        0,
