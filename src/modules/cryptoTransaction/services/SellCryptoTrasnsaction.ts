...
export class SellCryptoTransaction {
  ...
  public execute = async ({ userId, quantity }: IRequest): Promise<void> => {
    ...
    const transactions =
      await this.cryptoTransactionRepository.listAllInvestments({
        accountId: account?.id,
      });

    if (!transactions.length) throw new BaseError(422, "No transactions");
    ...
    this.accountRepository.updateBalance({
      account,
      value: quantityTotal * executionPrice,
    });

    await this.cryptoTransactionRepository.changeCryptoTransactionType({
      transactionsId: soldTransitions,
    });
  };
}
